...
namespace CodeHub.GitHub.Controllers.Readme
{
    public class ReadmeController : WebViewController
    {
        ...
        public ReadmeController(string user, string slug)
        {
            _user = user;
            _slug = slug;
            Title = "Readme";
        ...
        {
            var wiki = Application.Client.API.GetRepositoryReadme(_user, _slug).Data;
            var d = System.Text.Encoding.UTF8.GetString(Convert.FromBase64String(wiki.Content));
            var data = Application.Client.API.GetMarkdown(d);
